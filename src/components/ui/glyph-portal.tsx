...
/* eslint-disable react-hooks/purity -- GlyphPortal is a third-party scroll-driven component
   that intentionally uses imperative DOM manipulation inside useLayoutEffect. */
...
export default function GlyphPortal({
  ...
  const characters = Array.from(text).reduce<{ char: string; index: number }[]>((acc, char) => {
    const index = acc.length > 0 ? acc[acc.length - 1].index + acc[acc.length - 1].char.length : 0;
    acc.push({ char, index });
    return acc;
  }, []);
  ...
  return (
    <section ref={sectionRef} id={uid} className={className} aria-label={text}
      ...
      <style>{`
        ...
        ${q} [data-gp-enter]{display:inline-flex;align-items:center;gap:16px;min-height:44px;color:inherit;font:inherit;text-decoration:none;letter-spacing:inherit;border:none;cursor:pointer;background:transparent;padding:0;margin:0;}
