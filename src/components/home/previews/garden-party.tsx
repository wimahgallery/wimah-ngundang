...
export default function GardenPartyPreview({ play, active }: PreviewMotionProps) {
      ...
      <animated.div style={reveal} className="absolute inset-0">
        <animated.div style={drift} className="absolute -inset-y-5 inset-x-0">
          <MockPhoto
            ...
            alt="Preview template Garden Party"
            className="h-full w-full"
          />
        </animated.div>
      </animated.div>
      <span className="absolute inset-0 bg-gradient-to-b from-white/20 via-white/10 to-[var(--tpl-screen)]" />
      <MockGrain />
      <animated.div style={reveal} className="relative z-10 flex h-full flex-col items-center justify-end px-4 pb-6 text-center">
        <MockKicker className="text-[var(--tpl-ink)]/70">Garden Party</MockKicker>
